...
public class NoteServiceImpl implements NoteService {
    ...
    private final BuletinAutoGenerationServiceImpl buletinAutoGenerationService;

    public NoteServiceImpl(
        NoteRepository noteRepository,
        BuletinAutoGenerationServiceImpl buletinAutoGenerationService
        ) {
     ...
     */
    @Override
    public Note saveByBulletin(Note note) {
        note.setNoteC(note.getNoteI()*note.getMatiere().getCoeficient());
        Note result = noteRepository.saveAndFlush(note);
        log.debug("Request to save Note : {}", note);
        buletinAutoGenerationService.updateBulettin(result.getBulettin());
        return result;
    }
